...
MODEL_PATH=pretrained_models/Qwen3-VL-32B-Instruct
RUN_NAME=StAR_Qwen3VL32B_$(date +%Y%m%d_%H%M%S)

# Set the path to your Stage 1 LoRA checkpoint
STAGE1_LORA_CHECKPOINT=YOUR_STAGE1_CHECKPOINT_PATH  # e.g., star_workdir/StAR_Qwen3VL32B_.../global_step_XXX/actor/lora_adapter

# NOTE: 32B model requires 2 nodes (16 A6000 GPUs total).
# For multi-node training, set up a Ray cluster first, then run this script on the head node.
# Example:
#   [Head]   ray start --head --port=6379 --num-gpus=8
#   [Worker] ray start --address=HEAD_IP:6379 --num-gpus=8
#   Then run this script on the head node.
