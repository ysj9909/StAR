...
def convert_weight_keys(state_dict: dict[str, torch.Tensor], model: PreTrainedModel, base_sync_done = False):
    # convert state dict keys: https://github.com/huggingface/transformers/pull/38385
    # print("model class name:", model.base_model.model.__class__.__name__)
    # if "Qwen3VL" in model.base_model.model.__class__.__name__ and base_sync_done:
    #     for key, value in state_dict.items():
    #         state_dict = {k.replace("language_model.", ""): v for k, v in state_dict.items()}
    
    if (not hasattr(model, "_checkpoint_conversion_mapping")) or (model._checkpoint_conversion_mapping == {}):
        return state_dict
