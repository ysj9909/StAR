...
from .peft_model import PeftModel as HiraPeftModel
...
class HiraPeftModelForCausalLM(PeftModelForCausalLM):
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default", **kwargs) -> None:
        HiraPeftModel.__init__(self, model, peft_config, adapter_name, **kwargs)
        self.base_model_prepare_inputs_for_generation = self.base_model.prepare_inputs_for_generation


class HiraPeftModelForSeq2SeqLM(PeftModelForSeq2SeqLM):
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default", **kwargs) -> None:
        HiraPeftModel.__init__(self, model, peft_config, adapter_name, **kwargs)
        self.base_model_prepare_inputs_for_generation = self.base_model.prepare_inputs_for_generation
...
class HiraPeftModelForSequenceClassification(PeftModelForSequenceClassification):
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default", **kwargs) -> None:
        ...
        if hasattr(peft_config, "modules_to_save"):
            for name, _ in self.base_model.named_children():
                if any(module_name in name for module_name in self.modules_to_save):
                    ...
                    break
        from .peft_model import _set_trainable

        _set_trainable(self, adapter_name, module_names=getattr(peft_config, "modules_to_save", None))


class HiraPeftModelForTokenClassification(PeftModelForTokenClassification):
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default", **kwargs) -> None:
        HiraPeftModel.__init__(self, model, peft_config, adapter_name, **kwargs)
        classifier_module_names = ["classifier", "score"]
        if hasattr(peft_config, "modules_to_save"):
...
class HiraPeftModelForQuestionAnswering(PeftModelForQuestionAnswering):
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default", **kwargs) -> None:
        HiraPeftModel.__init__(self, model, peft_config, adapter_name, **kwargs)


class HiraPeftModelForFeatureExtraction(PeftModelForFeatureExtraction):
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default", **kwargs) -> None:
        HiraPeftModel.__init__(self, model, peft_config, adapter_name, **kwargs)
...
def get_peft_model(
    ...
    model_cls = MODEL_TYPE_TO_PEFT_MODEL_MAPPING[peft_config.task_type]
    
    # if isinstance(peft_config, PEFT_TYPE_TO_CONFIG_MAPPING["LORA"]) and (getattr(peft_config, "use_shira", False) or getattr(peft_config, "use_hira", False)):
    if getattr(peft_config, "use_shira", False) or getattr(peft_config, "use_hira", False):
        typed_map = {
            PeftModelForCausalLM: HiraPeftModelForCausalLM,
            PeftModelForSeq2SeqLM: HiraPeftModelForSeq2SeqLM,
            PeftModelForSequenceClassification: HiraPeftModelForSequenceClassification,
            PeftModelForTokenClassification: HiraPeftModelForTokenClassification,
            PeftModelForQuestionAnswering: HiraPeftModelForQuestionAnswering,
            PeftModelForFeatureExtraction: HiraPeftModelForFeatureExtraction,
        }
        model_cls = typed_map.get(model_cls, HiraPeftModel)
