...
class LoraConfig(PeftConfig):
    ...
    use_shira: bool = field(
        default=False,
        metadata={
            "help": (
                "Enable SculptHiRA variant."
            )
        },
    )
    use_hira: bool = field(
        ...
        metadata={
            "help": (
                "Enable (LoRA augmented) HiRA variant."
            )
        },
    )
    is_lora_augmented: bool = field(
        default=False,
        metadata={"help": "If True, use a second LoRA to augment the base weights in HiRA."},
    )
    r2: Optional[int] = field(
        default=None,
        metadata={"help": "Rank of the LoRA augment weights used in HiRA."},
    )
    shira_sparsity: float = field(
        default=1.0,
        metadata={"help": "Sparsity level for SHiRA mask, between 0 and 1."},
    )
    mask_multiplier: float = field(
        default=0.0,
        metadata={"help": "Mask multiplier for SHiRA mask, between 0 and 1."},
    )
    ...
    def __post_init__(self):
        ...
        if self.use_hira is False and self.is_lora_augmented:
            raise ValueError("`is_lora_augmented` can only be True when `use_hira` is enabled.")
        if self.is_lora_augmented and self.r2 is None:
            raise ValueError("`r2` must be specified when `is_lora-augmented` is True.")
        if self.r2 is not None and self.r2 <= 0:
            raise ValueError("`lora_augment_r` must be a positive integer.")
        ...
        if self.use_shira:
            if not 0.0 <= self.shira_sparsity <= 1.0:
                raise ValueError("`shira_sparsity` must be in [0, 1].")
