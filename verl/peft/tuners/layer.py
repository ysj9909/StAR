...
from .shira import compute_lora_weight
...
class LoraLayer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.use_hira: dict[str, bool] = {}   # added
        self.use_shira: dict[str, bool] = {}   # added
        self.is_lora_augmented: dict[str, bool] = {}   # added
        self.r2 = {}  # added
        self.shira_mask = nn.ParameterDict()   # added
    ...
    def update_layer(
        ...
        **kwargs,
        ...
        kwargs = locals().copy()
        del kwargs["self"]
        # print("use_hira:", use_hira, "is_lora_augmented: ", is_lora_augmented, "r2: ", r2)
        ...
        # if use_hira:
        #     print("hira weights are appropriately initialized!")
        ...
        if use_rslora:
            ...
            # if use_hira and is_lora_augmented:
            #         self.scaling[adapter_name] = lora_alpha / (r + r2)
            # else:
            self.scaling[adapter_name] = lora_alpha / r

        self.use_dora[adapter_name] = use_dora
        self.use_shira[adapter_name] = use_shira
        self.use_hira[adapter_name] = use_hira
        self.is_lora_augmented[adapter_name] = is_lora_augmented
    ...
    def _check_forward_args(self, x, *args, **kwargs):
        ...
        for adapter_name in unique_adapters:
            ...
            if self.use_dora.get(adapter_name, False) or self.use_hira.get(adapter_name, False):
                msg = "Cannot pass `adapter_names` when DoRA or HiRA is enabled."
                raise ValueError(msg)
...
class Linear(nn.Module, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.update_layer(
            ...
            use_dora=use_dora,
            lora_bias=lora_bias,
            use_shira=kwargs.get("use_shira", False),
            use_hira=kwargs.get("use_hira", False),
    ...
    def resolve_lora_variant(self, *, use_dora: bool, **kwargs) -> Optional[LoraVariant]:
        ...
        use_hira = kwargs.get("use_hira", False)
        if use_hira:
            from .variants import HiraLinearVariant
            print("- HiraLinearVariant is activated!")
            return HiraLinearVariant()
        if use_shira:
            from .variants import ShiraLinearVariant
            print("- ShiraLinearVariant is activated!")
            return ShiraLinearVariant()
        if use_dora:
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        # b_mod = self.lora_B[adapter]
        # a_mod = self.lora_A[adapter]
        
        # if b_mod.__class__.__name__ == "FullyShardedDataParallel" or a_mod.__class__.__name__ == "FullyShardedDataParallel":
        #     lora_weight = compute_lora_weight(a_mod, b_mod)
        #     return transpose(lora_weight, self.fan_in_fan_out) * self.scaling[adapter]

        # device = b_mod.weight.device
        # dtype = b_mod.weight.dtype
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                # if self.use_hira[active_adapter]:
