...
class DataProto:
    ...
    def __getitem__(self, item):
        ...
        # Case 1: List, numpy array, or torch tensor - use sel_idxs
        if isinstance(item, list | np.ndarray | torch.Tensor):
            return self.select_idxs(item)
        
        # Case 2: slice → slice를 정수 인덱스 배열로 변환 후 select_idxs 사용
        elif isinstance(item, slice):
            idxs = np.arange(len(self))[item]
            return self.select_idxs(idxs)
    ...
    def select_idxs(self, idxs):
        ...
        """
        # 1) idxs를 torch / numpy 두 버전으로 통일
        if isinstance(idxs, list):
            idxs_torch = torch.tensor(idxs)
            if idxs_torch.dtype != torch.bool:
                idxs_torch = idxs_torch.to(torch.long)
            idxs_np = idxs_torch.detach().cpu().numpy()
        elif isinstance(idxs, np.ndarray):
            idxs_np = idxs
            idxs_torch = torch.from_numpy(idxs_np)
        elif isinstance(idxs, torch.Tensor):
            idxs_torch = idxs
            # bool mask 아니고, float도 아닌 경우는 long index로
            if idxs_torch.dtype != torch.bool and not torch.is_floating_point(idxs_torch):
                idxs_torch = idxs_torch.to(torch.long)
            idxs_np = idxs_torch.detach().cpu().numpy()
        else:
            raise TypeError(f"Unsupported index type for DataProto.select_idxs: {type(idxs)}")

        is_bool = (idxs_np.dtype == bool)
        batch_size = int(idxs_np.sum()) if is_bool else idxs_np.shape[0]

        # 2) TensorDict 쪽은 그대로 tensor 인덱싱
        if self.batch is not None:
        ...
        # 3) non_tensor_batch에 대해서 타입별로 처리
        selected_non_tensor = {}
        for key, val in self.non_tensor_batch.items():
            # 3-1. numpy array면 numpy fancy indexing 유지
            if isinstance(val, np.ndarray):
                selected_non_tensor[key] = val[idxs_np]

            # 3-2. torch.Tensor면 torch 인덱싱
            elif torch.is_tensor(val):
                selected_non_tensor[key] = val[idxs_torch]

            # 3-3. list / tuple / 기타 시퀀스 타입
            else:
                if is_bool:
                    if len(val) != len(idxs_np):
                        raise ValueError(
                            f"Boolean index length {len(idxs_np)} does not match "
                            f"value length {len(val)} for key '{key}'."
                        )
                    selected_seq = [v for v, m in zip(val, idxs_np) if m]
                else:
                    selected_seq = [val[int(i)] for i in idxs_np]

                # *** 여기서부터가 중요: 타입을 유지한다 ***
                if isinstance(val, list):
                    selected_non_tensor[key] = selected_seq           # 그대로 list 유지
                elif isinstance(val, tuple):
                    selected_non_tensor[key] = type(val)(selected_seq)  # tuple로 다시 감싸기
                else:
                    # 기타 시퀀스: 최소한 리스트로
                    selected_non_tensor[key] = selected_seq
