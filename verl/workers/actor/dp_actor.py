...
class DataParallelPPOActor(BasePPOActor):
    ...
    def compute_log_prob(self, data: DataProto) -> torch.Tensor:
        ...
        log_probs_lst = []
        for micro_batch in tqdm(micro_batches, desc=desc, disable=(self.rank != 0)):
            micro_batch.to("cuda")
        ...
        for i, mini_batch in enumerate(mini_batches):
            ...
            for micro_batch in tqdm(micro_batches, desc=f"Update policy [{i + 1}/{n}]", disable=(self.rank != 0)):
                ...
                if loss_mode == "grpo":        
                    ...
                    pg_loss, pg_clipfrac, ppo_kl, _ = core_algos.compute_vanilla_policy_loss(
                        old_log_prob=old_log_prob,
                        log_prob=log_prob,
                        advantages=advantages,
                    ...
                    pg_loss, pg_clipfrac, ppo_kl = core_algos.compute_policy_loss_gpg(
                        # old_log_prob=old_log_prob,
                        log_prob=log_prob,
                        advantages=advantages,
                        response_mask=response_mask,
