...
def qwen2vl_dtensor_weight_loader(actor_weights: Dict[str, torch.Tensor], vllm_model: nn.Module) -> nn.Module:
    ...
    # distributed가 초기화되어 있지 않을 수 있으니 안전하게 래핑
    # def is_rank0():
    ...
    # if is_rank0():
    #     print("==== vllm_params keys (rank 0) ====")
    #     for name in vllm_params.keys():
    #         print(name)
    #     print("===================================")
    
    for actor_name, actor_weight in actor_weights.items():
        if actor_name.startswith("base_model.model."):
            actor_name = actor_name[len("base_model.model."):]
            
        # if "visual" not in actor_name and ".base_layer" in actor_name:
        #         actor_name = actor_name.replace(".base_layer", "")
        
        if "lora" in actor_name:
            continue
        ...
        for vllm_substr, hf_substr, shard_id in stacked_params_mapping:
            ...
            if clean_name.endswith(".bias") and clean_name not in vllm_params:
                continue  # skip loading extra bias for GPTQ models
            
            if "lora" in clean_name:
                continue

            if "visual" in clean_name:
                vllm_name = clean_name
            else:
                vllm_name = "language_model." + clean_name
                
            if vllm_name not in vllm_params:
                print(f"[WARN] `{vllm_name}` not in vLLM; skipping")
                continue
...
def qwen3vl_dtensor_weight_loader(actor_weights: Dict[str, torch.Tensor], vllm_model: nn.Module) -> nn.Module:
    ...
    for actor_name, actor_weight in actor_weights.items():
        ...
        # model.language_model.layers.35.mlp.down_proj.base_layer.weight
        ...
        for vllm_substr, hf_substr, shard_id in stacked_params_mapping:
            ...
            vllm_name = "language_model." + actor_name.replace(hf_substr, vllm_substr).replace("language_model.", "")
            if actor_name.endswith(".bias") and actor_name not in vllm_params:
                continue  # skip loading extra bias for GPTQ models
            ...
            if actor_name.startswith("model."):
                actor_name = actor_name[len("model."):]
            ...
            if "visual" in clean_name:
                ...
                vllm_name = "language_model.model." + clean_name.replace("language_model.", "")
...
def redistribute_dtensor(param_name: str, loaded_weights: DTensor, parallelize_plan: Dict = None):
    ...
    if parallelize_plan is not None:
        ...
        local_loaded_weights = loaded_weights.full_tensor() if isinstance(loaded_weights, DTensor) else loaded_weights
...
__MODEL_DTENSOR_WEIGHT_LOADER_REGISTRY__ = {
    ...
    "Qwen3VLForConditionalGeneration": qwen3vl_dtensor_weight_loader,
}
