...
from verl.utils.performance import log_gpu_memory_usage, simple_timer
from verl.workers.rollout.vllm_rollout import load_dtensor_weights
from verl.utils.vllm_utils import TensorLoRARequest, VLLMHijack, patch_vllm_moe_model_weight_loader
...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
                ...
                if self.layered_summon:
                    ...
                    with FSDP.summon_full_params(self.module, writeback=False):
                        if self.base_sync_done:
                            ...
                            full_sd = self.module.state_dict()
                            
                            for name, tensor in full_sd.items():
                                if any(x in name for x in ["_flat_param", "lora_"]):
                                    continue
                                
                                lora_params[name] = tensor
        ...
        with simple_timer("reshard", self.timing):
            ...
            params = convert_weight_keys(params, getattr(self.module, "_fsdp_wrapped_module", self.module), self.base_sync_done)
    ...
    def preprocess_data(self, data: DataProto) -> DataProto:
        if hasattr(vllm_ps, "get_tp_group") and self.is_qwen3:
            tp_group = vllm_ps.get_tp_group().device_group 
        else:
    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        if self.is_qwen3:
            ...
            load_dtensor_weights(updated_params, model)
