...
PE_TEMPLATE = "A cropped photo of {}"

def _expand_and_clamp(bbox: List[int], img_w: int, img_h: int, scale: float = 1.2) -> Tuple[int, int, int, int]:
    x1, y1, x2, y2 = bbox
    w = max(0, x2 - x1)
    h = max(0, y2 - y1)
    cx = x1 + w / 2.0
    ...
    nx2 = int(min(img_w, cx + new_w / 2.0))
    ny2 = int(min(img_h, cy + new_h / 2.0))
    # 최소 1픽셀 보장
    if nx2 <= nx1: nx2 = min(img_w, nx1 + 1)
    if ny2 <= ny1: ny2 = min(img_h, ny1 + 1)
...
def _clean_label(raw: str) -> str:
    """
    규칙:
      1) 모든 작은따옴표(') 제거 (어디에 있든 전부)
      2) 괄호 기호만 제거하고, 괄호 안 내용은 보존
      3) 언더바(_)는 공백으로 치환
      4) 끝자리 숫자(예: 'Horse 2') 제거
      5) 다중 공백 정리
    """
    s = (raw or "").strip()
    ...
    # 2) 괄호 기호만 제거(내용은 보존). 공백으로 바꾼 뒤 공백 정리.
    s = s.replace("(", " ").replace(")", " ")

    # 3) 언더바 -> 공백
    s = s.replace("_", " ")

    # 4) 끝자리 숫자 제거 (예: "Horse 2" -> "Horse")
    s = re.sub(r"\s*\d+$", "", s)
...
def main():
    ...
    # pe_model = pe.CLIP.from_config("PE-Core-L14-336", pretrained=True)
    ...
    if args.use_lora:
        ...
        # “lora_adapter” is the folder containing adapter_model.safetensors & adapter_config.json
        # peft_model = PeftModel.from_pretrained(
        #     reasoning_model,
        #     # args.reasoning_model_path,
        #     args.reasoning_model_path[:-len("huggingface")] + "lora_adapter",
        #     torch_dtype=torch.bfloat16,
        #     device_map="auto",
        # )
        adapter_path = args.reasoning_model_path[:-len("huggingface")] + "lora_adapter"
        lora_config = LoraConfig.from_pretrained(adapter_path)
        # config_dict = asdict(lora_config)
        # config_dict["runtime_config"] = LoraRuntimeConfig(ephemeral_gpu_offload=False)
        # lora_config = LoraConfig(
        #     # use_hira=True,
        #     # is_lora_augmented=False,
        #     # r2=8,
        #     **config_dict)
    ...
    # QUESTION_TEMPLATE = \
    #     "Please find \"{Question}\" with bbox(es) and point(s)." \
    #     "Reason about what the question intends in this image (not merely words in the text) and localize the target(s). " \
    ...
    # QUESTION_TEMPLATE = \
    #     "Please find \"{Question}\" with bbox(es) and point(s). " \
    #     "Reason about what the question intends in this image by comparing the difference between object(s) to find all object(s) that best match, and localize them. " \
    #     "Output the thinking process in <think> </think> and final answer in <answer> </answer> tags. " \
    #     "Self-check before <answer>: confirm that each predicted region is supported by the evidence stated in <think> and directly answers \"{Question}\"; if any mismatch is found, revise the prediction, then produce <answer>. " \
    #     "Output the bbox(es) and point(s) inside the target object(s) in JSON format. " \
    #     "i.e., <think> thinking process (step-by-step reasoning) here </think> " \
    #     "<answer>{Answer}</answer>"
    ...
    #     "Please find \"{Question}\" with bbox(es) and point(s). " \
    #     "Compare the difference between object(s) and find the most closely matched object(s). " \
    #     "Return ALL matching instances; if there are no matches, return an empty list. Double-check that nothing is missed or hallucinated. " \
    #     "Output the thinking process in <think> </think> and final answer in <answer> </answer> tags. " \
    #     "Output the bbox(es) and point(s) inside the interested object(s) in JSON format. " \
    #     "i.e., <think> thinking process (step-by-step reasoning) here </think> " \
    #     "<answer>{Answer}</answer> or <answer>[]</answer>"
            ...
            for id_idx in range(len(batch_output_text)):
                ...
                if args.visualization:
                    try:
                        ...
                        out_png = os.path.join(vis_dir, f"{ann_id}.png")
                        fig.savefig(out_png)
                        plt.close(fig)
                    except Exception as e:
